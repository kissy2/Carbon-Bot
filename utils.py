...
def pathfinder(start,target,shuffle=True,multipath=False):
	...
	while heap:
		...
		if path:
			...
			ret=[]
			for p in path:
				ret.append([[d[1],y,p[n]['interactives']] for n in range(len(p)-1) for y in g(False) for d in p[n][y].items() if d[0]==p[n+1]['_id']])
				...
				for k in node[0][-1][d]:
					if node[0][-2 if node[0][-1]['_id']!=start['_id'] else -1]['_id']!=k:
						if k in done:
							...
							neighbor=get(k)
							heap.insert(0,[node[0]+[neighbor],f(neighbor['coord'],target['coord'])])
...
def click(cellid,offx=0,offy=0,alt=0,direction=None):
	...
	print("click",cellid)
	app.click(coords=(x+floor(row*61.7)+(ceil(offx*2/3) if offx else 0), y+floor(15.35*line)+offy-8*alt))
	sleep(0.25)
...
def get_current_node(cond=0):
	print('get_current_node',useful['mapid'],useful['mypos'])
	projec={'walkable':1}
	if not cond:
		projec['fightcells']=1
	elif cond==2:
...
def check_spells(spells,buffs,mat):
	...
	while useful['infight'] and useful['fight']['ap']>2 and enter:
		...
		if useful['infight'] and useful['fight']['ap']>2:
			if useful['fight']['mp']:
				print("try use mp")
				if (path:=get_path(x0,y0,x1,y1,mat)):
					print("use mp",path[min(useful['fight']['mp'],len(path))-1])
...
def fight(call,first_fight=False):
	app.send_keystrokes('{VK_SHIFT}')
	print("call infight",call)
	if first_fight:
