...
def addMapComplementaryInformationsDataMessage(ans):
    try:
        ...
        for actor in ans["actors"]:
            if (not useful['contextualId'] or not useful['accountId']) and 'name' in actor and actor['name'] == useful[
                'name']:
                    ...
                    if elmentid in tmp and tmp[elmentid]["elementCellId"] \
                        ...
                        for imap in mapinfo:
                            if imap["id"] == ans["mapId"]:
                                useful["resources"][elmentid]["offset"] = imap['interactives']['2'][str(elmentid)][
                                    "offset"]
                    elif element["enabledSkills"][0]["skillId"] == 114 and element["onCurrentMap"]:
                        useful['zaap'] = True
                    elif element["enabledSkills"][0]["skillId"] == 355 and element["onCurrentMap"]:
                        useful['marketplace'] = str(elmentid)
                    elif element["enabledSkills"][0]["skillId"] == 211:
...
def read(type, data: Data):
    try:
        ...
        if ans["__type__"] == "GameFightOptionStateUpdateMessage":
            ...
            if ans['barType'] == 0:
                useful['shortcuts'] = [(x['slot'] + 1) % 10 for x in ans['shortcuts'] if
                                       x['slot'] < 10 and x['itemUID']]
        # elif ans['barType'] == 1:
        # 	useful['spells'] = {x['spellId']:x['slot'] for x in ans['shortcuts']}
            ...
            useful['inventory'][ans['object']['objectGID']] = {'objectUID': ans['object']['objectUID'],
                                                               'quantity': ans['object']['quantity']}
            ...
            if ans['genericId'] in useful['inventory']:
                ...
                useful['ready_to_sell'].append({'genericId': ans['genericId'], 'minimalPrices': ans['minimalPrices'],
                                                'averagePrice': ans['averagePrice'],
                                                'quantity': useful['inventory'][ans['genericId']]['quantity'],
                                                'type': 'r' if useful['mapid'] == 73400322 else 'c'})
            ...
            if useful['infight']:
                ...
                elif ans['targetId'] in useful['fight']['enemyteamMembers']:
                    useful['fight']['enemyteamMembers'][ans['targetId']]['lifepoints'] -= ans['loss']

        elif ans["__type__"] in (
                "NpcDialogCreationMessage", "LockableShowCodeDialogMessage", "ZaapDestinationsMessage"):
            useful["dialog"] = True
        ...
        elif ans["__type__"] == "GameRolePlayMonsterNotAngryAtPlayerMessage" and useful['contextualId'] == ans[
            'playerId']:
        ...
        # print('remove',ans,useful['agro'])
            ...
            try:
                ...
                if useful['contextualId'] != ans['contextualId']:
                    useful["map_players"][ans["contextualId"]] = {"name": ans["name"],
                                                                  "cellid": ans["disposition"]["cellId"]}
                else:
                    useful["mypos"] = ans["disposition"]["cellId"]
            except:
                logging.error(f'ok bb {ans}')

        elif ans["__type__"] == "GameContextActorPositionInformations" or ans[
            "__type__"] == "GameContextActorInformations" or ans["__type__"] == "GameRolePlayNamedActorInformations":
            if not useful['contextualId'] and ans["__type__"] == "GameRolePlayNamedActorInformations" and useful[
                "name"] == ans["name"]:
        ...
        # logging.warning(f'Sender : {ans["senderName"]}\nContent : {str(ans["content"])}\nChannel : {ans["channel"]}')
            ...
            useful['dialog'] = True
                ...
                "ExchangeRequestedMessage", "ExchangeShopStockStartedMessage", "GameRolePlayPlayerFightRequestMessage",
                "ExchangeRequestedTradeMessage", "PartyInvitationMessage", "GuildInvitedMessage",
                "InviteInHavenBagOfferMessage", "ExchangeReplyTaxVendorMessage"):
            useful["dialog"], useful['threat'] = True if ans["__type__"] not in (
                "ExchangeReplyTaxVendorMessage", "PartyInvitationMessage") else 2, 'low'
                        ...
                        for member in ans["team"]["teamMembers"]:
                            if 'grade' in member:
                                try:
                                    ...
                                    useful["fight"]["enemyteamMembers"][int(member["id"])]["monsterId"] = member[
                                        "monsterId"]
                                except:
