def launch_in_process(conn, client, name, server, parameters):
    ...
    collection, wait, useful['name'], useful['server'], useful['mod'], connected = MongoClient(
        'localhost:27017').admin, lambda x, y: agro(uniform(x, y)), name, server, False, True
    func, waitp, archmonsters = lambda x: collection.nodes.find_one({'mapid': x}, {'coord'}), b'', {
        x['raceid']: x['_id'] for x in collection.archmonsters.find({})}
    logging.basicConfig(format='%(asctime)s %(levelname)s %(message)s', filename=f'logs/log-{name} - {server}.txt',
                        level=logging.INFO)
    ...
    try:
        useful['tbd'] = time() + 60 * (
                    randint(int((tmp := parameters[6][:parameters[6].find('|')].split('-'))[0]), int(tmp[1])) + 5)
    ...
    def click(cellid, offx=0, offy=0, alt=0, direction=None, c=1, s=0, so=.5):
        try:
            ...
            limit, x, y, line, row = {'s': lambda x, odd: (x[0], x[1] + 5) if odd else (x[0], x[1] + 20),
                                      'n': lambda x, odd: (x[0], x[1] - 27) if odd else (x[0], x[1] - 14),
                                      'w': lambda x, odd: (x[0] - 50, x[1]) if odd else (x[0] - 23, x[1]),
                                      'e': lambda x, odd: (x[0] + 23, x[1]) if odd else (
                                      x[0] + 50, x[1])}, 269, 16, cellid // 14, cellid % 14
            if odd := line % 2:    x += 31
            ...
            elif direction == 'd' and useful['mapid'] in (128453121, 128452097, 128451073):
                offx, offy = 10, 15
            logging.info(
                f'Click : {cellid}, {offx}, {offy}, {alt}, {direction} , {x + ceil(row * 62.4) + floor(offx * .687)} , {y + floor(15.58 * line) + ceil(offy * .55) - 9 * alt + 38}')
            send(b'c\n' + (x + ceil(row * 62.4) + floor(offx * .687)).to_bytes(2, 'big', signed=True) + b'\n' + (
                        y + floor(15.58 * line) + ceil(offy * .55) - 9 * alt).to_bytes(2, 'big',
                                                                                       signed=True) + b'\n' + c.to_bytes(
                2, 'big'))
            if s: wait(s, s + so)
        except:
            logging.error(
                f'Error in click cellid : {cellid} , offx : {offx} , offy : {offy} , altitude : {alt} , direction : {direction}',
                exc_info=1)
    ...
    def get_mod_state():
        try:
            return urlopen(Request(
                "https://panel.snowbot.eu/api/moderatorCheckerPC/checkModerator.php?gameServer=" + useful["server"]),
                           timeout=1).read() != b'false'
        except:
    ...
    def pathfinder(start, target, shuffle=True, multipath=False):
        ...
        f, g, get = lambda s, t: 4 * (
                    abs(t[0] - s[0] if t[0] > 0 and s[0] > 0 or t[0] < 0 and s[0] < 0 else abs(t[0]) + abs(s[0])) + abs(
                t[1] - s[1] if t[1] > 0 and s[1] > 0 or t[1] < 0 and s[1] < 0 else abs(t[1]) + abs(s[1]))), lambda \
                        x: sample(('n', 's', 'w', 'e', 'd'), 5) if x else ('n', 's', 'w', 'e', 'd'), lambda \
                        x: collection.nodes.find_one({'_id': x},
                                                     {'coord': 1, 'mapid': 1, 'n': 1, 's': 1, 'w': 1, 'e': 1, 'd': 1})
        ...
        while heap:
            ...
            for node in nodes:
                for d in g(shuffle):
                    if d == 'd':    node[0][-1][d] = {x: y for x, y in node[0][-1][d].items() if
                                                      y != -1}  # remove unlinked doors
                    for k in node[0][-1][d]:
                        if node[0][-2 if node[0][-1]['_id'] != start['_id'] else -1]['_id'] != k:
                            if k in done:
                                ...
                                heap.insert(0, [node[0] + [neighbor := get(k)],
                                                len(node[0]) + f(neighbor['coord'], target['coord'])])
        ...
        try:
            ...
            for n in collection.nodes.find({'mapid': mapid}, projec):
                if not cond or pos in n['walkable']:
                    break
            return n['_id'] if cond == 1 else {'interactives': n['interactives'],
                                               'walkable': n['walkable']} if cond == 2 else (
            n['_id'], n['d']) if cond == 3 else n['fightcells']
        except:
            logging.error(
                f'Error in get_current_node cond : {cond} mapid : {mapid} pos: :{pos} , current mapId {useful["mapid"]} nested {nested}',
                exc_info=1)
            if connected:
                if nested:
    ...
    def get_path(x0, y0, x1, y1, mat, revert=True):
        try:
            f, done, offset = lambda x0, y0: abs(x0 - x1) + abs(y0 - y1), {revert_coord(x0, y0)}, (
            (0, 1), (0, -1), (-1, 0), (1, 0))
            heap = [[[(x0, y0)], f(x0, y0)]]
            while heap:
                ...
                for i in best:
                    ...
                    for j in range(4):
                        if not (ooe := reverted // 14 % 2) and not reverted % 14 and j in (
                        1, 2) or ooe and reverted % 14 == 13 and j in (0, 3) or reverted // 14 == 39 and j in (1, 3):
                            continue
                        d = (i[0][-1][0] + offset[j][0], i[0][-1][1] + offset[j][1])
                        if (cellid := revert_coord(d[0], d[1])) > -1 and cellid < 560 and mat[cellid] == 2 and d not in \
                                i[0] and c not in temp and c not in done:
    ...
    def get_closest_enemy(x0, y0, heap, mat, area, hm=None):
        try:
            ...
            temp = [[x, (t := switch_coord(y['cellid']))[0], t[1]] for x, y in
                    useful['fight']['enemyteamMembers'].items()]
            temp.sort(key=lambda x: abs(x0 - x[1]) + abs(y0 - x[2]))
            for _ in range(area):
                for cell in heap[start:]:
                    for i in range(4):
                        if not ((ooe := cell // 14 % 2) and cell % 14 == 13 and i in (
                        1, 2) or not ooe and not cell % 14 and i in (0, 3)) and (next_cell := (to_add[
                                                                                                   i] + 1 if ooe and i < 2 else
                        to_add[i] - 1 if not ooe and i > 1 else to_add[i]) + cell) < 560 and next_cell > -1 and mat[
                            next_cell] == 2 and next_cell not in heap:
                            heap.append(next_cell)
                    start += 1
            heap = [(switch_coord(x), x) for x in heap]
            t = [[x, [(y[-1] if insight(y[0][0], y[0][1], x[1], x[2], mat) else y[-1] * -1, y[0][0], y[0][1]) for y in
                      heap]] for x in temp[:hm]]
    ...
    def check_spells(spells, buffs, mat, treasure, hpc, lvl_cond):
        try:
            ...
            logging.info(
                f"Turn ({useful['fight']['round']}) : {useful['fight']['enemyteamMembers']} , Ap : {useful['fight']['ap']} , Mp : {useful['fight']['mp']} {useful['lifepoints']} {useful['maxLifePoints']}")
            while useful['infight'] and useful['fight']['ap'] > 2 and enter and not heal:
                (x0, y0), ok1 = switch_coord(p := useful['mypos']), False
                for y, x, k, v in ((y, x, k, v) for y in get_closest_enemy(x0, y0, [p], mat, useful['fight']['mp']) for
                                   k, v in spells.items() for x in y[1]):
                    if useful['infight'] and useful['fight']['ap'] > 2:
                        if y[0][0] in useful['fight']['enemyteamMembers'] and (
                                v['range'][1] + (useful['fight']['range'] if v['range'][2] else 0) >= (
                        calc := abs(x[1] - y[0][1]) + abs(x[2] - y[0][2])) and calc >= v['range'][0] and not v[
                            'fc'] and (not v['inline'] or x[1] == y[0][1] or x[2] == y[0][2]) and (
                                        not v['sight'] or x[0] > -1) and v['ap'] <= useful['fight']['ap']):
                            if treasure and 'hunt' in useful and useful['fight']['enemyteamMembers'][-1][
                                'lifepoints'] <= hpc:
                            ...
                            counter = 3
                            while useful['infight'] and y[0][0] in useful['fight']['enemyteamMembers'] and counter and \
                                    v['ap'] <= useful['fight']['ap']:
                                logging.info(f"enemies {useful['fight']['enemyteamMembers']}")
                                logging.info(f'my health {useful["lifepoints"]} {useful["maxLifePoints"]}')
                                if treasure and 'hunt' in useful and useful['fight']['enemyteamMembers'][-1][
                                    'lifepoints'] <= hpc and useful['lifepoints'] / useful['maxLifePoints'] < .8:
                                    logging.info(f'waiting treasure regen')
                                    break
                                if treasure and 'hunt' in useful and useful['lifepoints'] / useful[
                                    'maxLifePoints'] < .25:
                                ...
                                logging.info(
                                    f" Ap : {useful['fight']['ap']} , Mp : {useful['fight']['mp']} {useful['lifepoints']} {useful['maxLifePoints']}")
                ...
                if useful['infight'] and not ok1:
                    if (jump_cond := not buffs['1']['fc'] and buffs['1']['ap'] <= useful['fight']['ap']) or heal:
                        ...
                        for y, x, v in ((y, x, v) for y in get_closest_enemy(x0, y0, [p], mat, 50, 1) for x in y[1] for
                                        v in spells.values()):
                            calc = abs(x[1] - y[0][1]) + abs(x[2] - y[0][2])
                            if mat[abs(x[0])] == 2 and x[0] > -1 and v['range'][1] + (
                            useful['fight']['range'] if v['range'][2] else 0) >= calc and (
                                    not v['inline'] or x[1] == y[0][1] or x[2] == y[0][2]) and x[0] != \
                                    useful['fight']['enemyteamMembers'][y[0][0]]['cellid'] and calc < m:
                                m, x1, y1 = calc, x[1], x[2]
                                if heal and jump_cond or move_fight(
                                        (t := get_path(x0, y0, x1, y1, mat))[min(useful['fight']['mp'], len(t) - 1)]) != \
                                        t[-1] and jump_cond:
                                    press('{VK_CONTROL DOWN}')
                                    press('1', s=0.8, so=1)
                                    press('{VK_CONTROL}')
    ...
    def fight(treasure=False):
        try:
            ...
            try:
                ...
                if useful['mount'] and 'riding' in useful['mount'] and not useful['mount']['riding']:    press(
                    '{VK_NUMPAD6}')
            except:
                logging.error('lock spec mount error', exc_info=1)
            try:
                mat, prev, mapid, hpc = get_current_node(), useful['mypos'], useful['mapid'], \
                                        useful['fight']['enemyteamMembers'][-1]['lifepoints'] * .12
            except:
            ...
            try:
                m = 600
                for c in useful['fight']['positionsForDefenders'] if useful['fight']['defenderId'] == useful[
                    'contextualId'] else useful['fight']['positionsForChallengers']:
                    (x0, y0), (x1, y1) = switch_coord(useful['fight']['enemyteamMembers'][-1]['cellid']), switch_coord(
                        c)
            ...
            spells, buffs = (
                ...
                {'1': {'range': (1, 5, False), 'ap': 4, 'cpt': 1, 'recast': 0, 'fc': 0, 'sight': True, 'inline': True},
                 '2': {'range': (1, 4, False), 'ap': 5, 'cpt': 2, 'recast': 0, 'fc': 0, 'sight': True, 'inline': False},
                 '3': {'range': (1, 3, False), 'ap': 3, 'cpt': 2, 'recast': 0, 'fc': 0, 'sight': True, 'inline': False},
                 '4': {'range': (1, 6, False), 'ap': 4, 'cpt': 1, 'recast': 0, 'fc': 0, 'sight': False,
                       'inline': False}}
                ...
                {'1': {'range': (1, 5, False), 'ap': 4, 'cpt': 1, 'recast': 0, 'fc': 0, 'sight': True, 'inline': True},
                 ...
                 '5': {'range': (1, 6, False), 'ap': 4, 'cpt': 1, 'recast': 0, 'fc': 0, 'sight': False,
                       'inline': False}}
                ,
                # buffs
            ...
            while useful['infight'] and connected:
                ...
                press('{F1}')
                wait(2, 3)
                while 'turn' in useful['fight'].keys() and not useful['fight']['turn'] and connected and useful[
                    'infight'] and (counter := counter - 1):
    ...
    def move(nid, zaap=False, exit=None, nested=False, harvest=False, passed=False, hint_sleeping=False):
        ...
        if zaap:
            ...
            for x in collection.zaaps.find({}):
                if (
                calc := abs(int((c := x['_id'].split(','))[0]) - n['coord'][0]) + abs(int(c[1]) - n['coord'][1])) < m:
                    name, m = x['name'], calc
    ...
    def randomteleport():
        try:
            ...
            temp = [68552706, 88213271, 88212759, 88212758, 88212246, 88212247, 88080662, 73400320, 73531904, 73400321,
                    73400322, 73400323, 84935175, 147768, 147766, 145203, 146741, 146748, 144419, 144931]
            ...
            if useful['map_merchant']:
                ...
                for x in static_interactives:
                    if x == useful['marketplace']:
                        ...
                        break
            if 'wait' in useful:
                agro(randint(50, 65) * useful['wait'])
            else:
                wait(200, 500)
        except:
    ...
    def treasure_hunt(supervised=False, harvest=False):
        ...
        def wait_fix():
            ...
            while (t :=
            Popen(args='REG QUERY HKCU\Environment /v fix', stdout=PIPE, stderr=DEVNULL).communicate()[0].split(b' ')[
                -1].decode())[:t.index('\r')] != name + '-' + server:    agro(10)
                    ...
                    while i and (cur := collection.nodes.find_one({'_id': cur['_id']}))[d[1][0]]:
                        ...
                        if x != x1 or y != y1:
                            if temp2 := collection.treasure.find_one({'_id': (id := f'{x1},{y1}')}):
                                if directions[d[0]][-1] in temp2:
                                    for t in temp2[directions[d[0]][-1]]:
                                        if t['id'] == hint['poiLabelId']:
                                            if direction == directions[d[0]][-1] and test_coord(start_coord[0],
                                                                                                start_coord[-1], x1, y1,
                                                                                                direction):
                                                t['x'], t['y'] = x, y
                                ...
                                collection.treasure.update_one({'_id': id}, {
                                    '$set': {directions[d[0]][-1]: temp2[directions[d[0]][-1]]}})
                            else:
                                collection.treasure.insert_one({'_id': id}, {
                                    '$set': {directions[d[0]][-1]: [{'id': hint['poiLabelId'], 'x': x, 'y': y}]}})
                        i -= 1
                with open('./hints.json', 'r', encoding='utf8') as f:
                    a = load(f) + [{'coord': f'{x},{y}',
                                    'name': collection.named_ids.find_one({'_id': hint['poiLabelId']})['name'],
                                    'direction': direction, 'start': start_coord}]
        ...
        def fix():
            try:
                ...
                check_hint()
                last, current_coord, prev, j = (t := collection.nodes.find_one({'mapid': useful['hunt']['flags'][-1][
                    'mapId'] if useful['hunt']['flags'] else useful['hunt']['startMapId']}, {'mapid', 'coord'}))['_id'], \
                                               t['coord'], useful['hunt']['currentstep'], len(useful['hunt']['flags'])
                ...
                if supervised:
                    ...
                    while useful['hunt']['availableRetryCount']:
                        ...
                        last = l[0] if (l := [*collection.nodes.find_one({'_id': last}, {
                            directions[useful['hunt']['currentstep']['direction']][0], 'coord'})[
                            directions[useful['hunt']['currentstep']['direction']][0]]]) else [*nwmd][
                            randint(0, len(nwmd) - 1)] if (
                            nwmd := collection.nodes.find_one({'_id': last}, {"d", 'coord'})[
                                "d"]) else -1  # temp solution randint to avoid cycle in multi doors node
                        if last == -1:
                            logging.critical(f"Force retake in {useful['mapid']} hunt {useful['hunt']}")
                            notify(bytes(
                                f"TREASER HUNT\nRETAKE {useful['server'], useful['name'], useful['mapid'], useful['hunt']['currentstep']}",
                                'utf-8'))
                            useful["retake"] = True
                            del useful["hunt"]
                            return
                            ...
                            move(last)
                        logging.info(
                            f"Fix {last} {directions[useful['hunt']['currentstep']['direction']][0]} {useful['hunt']}")
                        check_hint(j)
        ...
        def f(coord, last, d, nested=False):
            ...
            cur, ret, cont = collection.nodes.find_one({'_id': last},
                                                       t := {d, 'd', 'coord', 'mapid', 'walkable'}), [], 0
            ...
            while (cur['coord'] != coord and not ret) or cont:
                ...
                if not calc:
                    ...
                    if not ret and best == -1:
                        if not nested:
                            for x in collection.nodes.find(
                                    {'mapid': collection.nodes.find_one({'_id': last}, {'mapid'})['mapid']}, {'_id'}):
        ...
        e, e1, e2, prev = get_current_node(1, 142089230, 357), get_current_node(1, 128452097, 489), get_current_node(1,
                                                                                                                     142088718,
                                                                                                                     356), None
        ...
        while connected and useful['th_counter']:
            try:
                ...
                if time() > useful['tbd']:
                    ...
                    break
                if (t :=
                Popen(args='REG QUERY HKCU\Environment /v dofus_stop', stdout=PIPE, stderr=DEVNULL).communicate()[
                    0].split(b' ')[-1].decode())[:t.index('\r')] == '1':    return
                ...
                if 'hunt' not in useful:
                    ...
                    wait(4, 8)
                    prev = useful['hunt']
                directions, last = {0: ('e', 'right', '4'), 4: ('w', 'left', '0'), 2: ('s', 'bottom', '6'),
                                    6: ('n', 'top', '2')}, move(
                    useful['hunt']['flags'][-1]['mapId'] if useful['hunt']['flags'] else useful['hunt']['startMapId'],
                    True, exit=e)
                while useful['hunt']['checkPointTotal'] - useful['hunt']['checkPointCurrent'] - 1:
                    for j in range(len(useful['hunt']['flags']), useful['hunt']['totalStepCount']):
                        current_coord, skip, fail = collection.nodes.find_one({'_id': last}, {'coord': 1, '_id': 0})[
                                                        'coord'], False, False
                        ...
                        if 'poiLabelId' in useful['hunt']['currentstep']:
                            ...
                            if res := collection.treasure.find_one({
                                                                       '_id': f'{strcell + "*" + str(int(useful["mapid"])) if current_coord[0] == -15 and current_coord[1] == 39 else strcell}',
                                                                       f'{(d := str(useful["hunt"]["currentstep"]["direction"]))}.id':
                                                                           useful['hunt']['currentstep']['poiLabelId']},
                                                                   {d}):
                                for x in res[d]:
                                    if x['id'] == useful['hunt']['currentstep']['poiLabelId']:
                                        next_node = f([x['x'], x['y']], last,
                                                      directions[useful['hunt']['currentstep']['direction']][0])
                            ...
                            wait(2, 5)
                            temp, t, tt, ten = last, {directions[useful['hunt']['currentstep']['direction']][0]: 1,
                                                      "d": 1}, {
                                                   directions[useful['hunt']['currentstep']['direction']][0]: 1, "d": 1,
                                                   "walkable": 1}, 10
                            try:
                                while ten:
                                    calc = 0
                                    for y in t:
                                        ...
                                        for x in collection.nodes.find({'mapid': useful['mapid']}, t):
                                            if x['_id'] != temp and (
                                                    x[directions[useful['hunt']['currentstep']['direction']][0]] or x[
                                                'd']):  # vulnerable to 3 + nodes map
                        ...
                        if (length := len(next_node) == 1) and next_node[0] == last:
                            ...
                            hint_sleeping = True
                            for x in next_node:
                                last, prev = move(x, harvest=harvest, hint_sleeping=hint_sleeping), len(
                                    useful['hunt']['flags'])
                                hint_sleeping = False
                ...
                if useful['hunt']['result'] != 4:
                    ...
                    if useful['mount'] and 'riding' in useful['mount'] and useful['mount']['riding']:    press(
                        '{VK_NUMPAD6}')
                else:
                    randomteleport()
    ...
    def get_closest(edge, mat, direction):
        try:
            ...
            check = lambda cell, index: not (
                        cell % 14 == 13 and index in (5, 6) or not ooe and not cell % 14 and index in (
                4, 7) or not cell % 14 and index == 3 or cell % 14 == 13 and index == 2)
            calc = lambda cell, index: temp if (temp := (base[index] + 1 if ooe and index in (4, 5) else base[
                                                                                                             index] - 1 if not ooe and index > 5 else
            base[index]) + cell) > -1 and temp < 560 else False
            if direction == 'd':
                return generate(edge, direction)
            elif useful['mypos'] in edge:
                return (
                y if (y := useful['mypos'] + (x if x < 3 else -x + 2) * (28 if direction in ('w', 'e') else 1)) else
                useful['mypos'] for x in range(5))  # check new pos in node edges
            ...
            while l:
                ...
                for i in range(8):
                    if not check(l[0], i):
                        continue
                    if not i and not (
                            (t := calc(l[0], 4)) and mat[t] == 2 and check(l[0], 4) or (t := calc(l[0], 5)) and mat[
                        t] == 2 and check(l[0], 5)):
                        continue
                    if i == 1 and not (
                            (t := calc(l[0], 7)) and mat[t] == 2 and check(l[0], 7) or (t := calc(l[0], 6)) and mat[
                        t] == 2 and check(l[0], 6)):
                        continue
                    if i == 2 and not (
                            (t := calc(l[0], 6)) and mat[t] == 2 and check(l[0], 6) or (t := calc(l[0], 5)) and mat[
                        t] == 2 and check(l[0], 5)):
                        continue
                    if i == 3 and not (
                            (t := calc(l[0], 7)) and mat[t] == 2 and check(l[0], 7) or (t := calc(l[0], 4)) and mat[
                        t] == 2 and check(l[0], 4)):
                        continue
                ...
                l.pop(0)
            logging.warning(
                f'out empty in get_closest (cells) current pos {useful["mypos"]} edges : {edge}\nmat : {mat}\ndirection : {direction}')
            return generate(edge, direction)
        except:
    ...
    def check_adjc(l, mat, pos):
        try:
            ...
            while l:
                for i in range(5):
                    if (ooe := l[0] // 14 % 2) and l[0] % 14 == 13 and i in (1, 2) or not ooe and not l[
                                                                                                          0] % 14 and i in (
                    0, 3):
                        continue
                    next_cell = (base[i] + 1 if ooe and i < 2 else base[i] - 1 if not ooe and (i > 1 and i < 4) else
                    base[i]) + l[0]
        ...
        try:
            ...
            if not useful['energy']:
                ...
                while not useful['energy'] and counter:
                    ...
                    click(collection.nodes.find_one({'_id': move(
                        collection.nodes.find_one({'mapid': useful['phenix']}, {'_id'})['_id'], passed=True)}, {'o'})[
                              'o'][useful['phenix_id']], offx=7, offy=-4, s=20)
                    counter -= 1
                if not counter:
                ...
                if cond[2] != -1:
                    if tries >= cond[2]:
                        ...
                        if useful[cond[0]] == cond[1]:
                            ...
                            if not cond[2]:
                                logging.critical(
                                    f'Breaking from path : something bad happened here {useful["mapid"]} {len(buf)} {len(waitp)}')
                                buf.reset()
                                waitp = b''
                                return -1
            ...
            try:
                for n in p:
                    ...
                    while loop:
                        ...
                        for x, y in temp:
                            logging.info(f'{x} {y}')
                            if 'alllevel' in y and y['alllevel'] < useful['my_level'] * threshold and y[
                                'cellId'] not in [c['cellId'] - 28 * r for c in useful['map_players'].values() for r in
                                                  range(2)] and y['cellId'] not in [(c['cellId'] if not mlr else c[
                                                                                                                     'cellId'] - (
                                                                                                                 14 if (
                                                                                                                 ooe :=
                                                                                                                 c[
                                                                                                                     'cellId'] // 14 % 2) else 15) if mlr == 1 else
                            c['cellId'] - (13 if ooe else 14)) - 28 * r for c in useful['map_npc'].values() for mlr in
                                                                                    range(3) for r in range(4)]:
    ...
    def collect(rsc, priority, exceptions, safe_mode, key=None):
        try:
            ...
            for c, k in order([(x['enabledSkill'], x['elementCellId'], k) for k, x in useful['resources'].items() if
                               (str(x['elementCellId']) in e and 'enabledSkill' in x and x['enabledSkill'] in rsc)],
                              mat, priority):
                if safe_mode and len(useful['map_players']):    break
                if k in useful['resources'] and useful['resources'][k]['elementCellId'] not in [c['cellId'] - 28 * r for
                                                                                                c in useful[
                                                                                                    'map_mobs'].values()
                                                                                                for r in range(2)] and \
                        useful['resources'][k]['elementCellId'] not in [(c['cellId'] if not mlr else c['cellId'] - (
                14 if (ooe := c['cellId'] // 14 % 2) else 15) if mlr == 1 else c['cellId'] - (
                13 if ooe else 14)) - 28 * r for c in useful['map_mobs'].values() for mlr in range(3) for r in
                                                                        range(4)]:
                    maxi, offx, offy, prev = 5, e[(s := str(c))]['xoffset'], e[s]['yoffset'], useful['mypos']
                    if (t := useful['resources'][k]['enabledSkill']) in exceptions:    offx, offy = offx + \
                                                                                                    exceptions[t][
                                                                                                        0], offy + \
                                                                                                    exceptions[t][1]
                    click(c, offx, offy, e[s]['altitude'], s=.5)
                    while k in useful['resources'] and maxi:
                        if safe_mode and len(useful['map_players']) and not useful['infight']:
                            return mat
                        elif (t := wait_check_fight(1, 2.2)) == 100 or not useful['Harvesting'] and prev == useful[
                            'mypos']:
                            break
                            ...
                            notify((
                                       f'MISS CLICK WHILE HARVESTING\n{name + "-" + server} fix offset {useful["mapid"]} : {c}').encode())
                            break
                        maxi -= 1
            return mat
        except:
            logging.error(
                f'Eroor in collect rsc : {rsc}\nkey : {key}\npriority : {priority}\nexceptions : {exceptions}\nuseful : {useful}',
                exc_info=True)
    ...
    def check_inventory():
        try:
            ...
            if useful["full_inventory"] or useful['inventory_weight'] / useful['inventory_max'] > .97:
                ...
                for x in sample((84935175, 99095051, 192415750, 8914959, 8913935, 8912911, 2885641, 2884617, 2883593,
                                 8129542, 91753985, 54534165), 12):
            ...
            while connected:
                for p in sample(paths, len(paths)):
                    ...
                    for e in path:
                        ...
                        if not connected or cond_wait(
                            ['mapid', prev, 4, wrapper(click, cell, direction=e[1]), e[2], e[3]]) == -1:    break
                    else:
    ...
    def all_cells():
        try:
            ...
            clean = []
            merchants_cells = [c['cellId'] for c in useful['map_merchant'].values()]
            static_interactives = [c - 28 * r for c in cmap['o'].values() for r in range(2)] + [(c if not mlr else c - (
                14 if (ooe := c // 14 % 2) else 15) if mlr == 1 else c - (13 if ooe else 14)) - 28 * r for c in
                                                                                                cmap['o'].values() for
                                                                                                mlr in range(3) for r in
                                                                                                range(4)]
            for c in cmap['walkable']:
                t = False
                for i in ('n', 's', 'w', 'e', 'd'):
                    for n in cmap[i].values():
                        if n != -1 and c not in n and c not in static_interactives and c not in clean and c not in cmap[
                            'interactives'] and c not in merchants_cells:
                            clean.append(c)
                        else:
                            t = True
                            break
    ...
    def clean_cells(cmap, clean, cond=False):
        try:
            ...
            all_clean_cells = [c for c in clean if
                               c not in [c['cellId'] - 28 * r for i in tup for c in useful[i].values() for r in
                                         range(2)] + [(c['cellId'] if not mlr else c['cellId'] - (
                                   14 if (ooe := c['cellId'] // 14 % 2) else 15) if mlr == 1 else c['cellId'] - (
                                   13 if ooe else 14)) - 28 * r for i in tup for c in useful[i].values() for mlr in
                                                      range(3) for r in range(5)] + [
                                   c['cellId'] if 'cellId' in c else c['cellid'] for c in
                                   useful['map_players'].values()]]
            ...
            logging.error(
                f'Error in clean cells {cmap} , {clean}  , {useful["map_players"]}, {useful[tup[0]]} , {useful[tup[1]]}',
                exc_info=1)
    ...
    def seller():
        ...
        def checker(cell, arg):
            ...
            click(47, 30, 25, s=3)
            for i in range(10):
                prev = deepcopy(useful['inventory'])
                for j in range(5):
                    click(108, j * 62 - 6, i * 78 - 13, s=.25, so=.75)
                if useful['inventory'] == prev:
                    break
            press('{VK_ESCAPE}', s=3)
        ...
        for x in useful['ready_to_sell']:
            if x['type'] == 'r':
                price = x['minimalPrices'][0] * .95 if x['quantity'] < 10 else x['minimalPrices'][1] * .97 / 10 if x[
                                                                                                                       'quantity'] < 100 else \
                    x['minimalPrices'][2] * .99 / 100
                if price == 0:
                    rate = 69
                else:
        ...
        # move to uncrowded map 88212246
        # move(88212246,True)
    ...
    def hdv():
        from subprocess import Popen, PIPE, DEVNULL
        useful['queringhdv'] = True
        while True:
            for res in currentinterestingresources:
                resname = res
                click(99, 0, 0, s=1, c=3)
                press('{BACKSPACE}')
                for y in resname:    press(y)
                sleep(random.randint(100, 330) / 100)
                click(131, -20, -20, s=2, c=1)
                if (t :=
                Popen(args='REG QUERY HKCU\Environment /v dofus_stop', stdout=PIPE, stderr=DEVNULL).communicate()[
                    0].split(b' ')[-1].decode())[:t.index('\r')] == '1':    return
    ...
    def enter_heaven_bag():
        press('h', s=10)
        if useful['mapid'] != 162793472:
            logging.warning(f'Could Not Teleport To HavenBag From This Map {useful["mapid"]} exit node {exit}')

    def move_to_closest_bank():
        # currently to bonta
        # n, m, counter = func(useful['mapid'])['coord'], 1000, 5
        # for x in sample((84935175, 99095051, 192415750, 8914959, 8913935, 8912911, 2885641, 2884617, 2883593,
        #                  8129542, 91753985, 54534165), 12):
        ...
        #         name, m = x, calc
        # print('args ',name, m)
        if not move('6257', False):
            logging.info(f'Failed going to bank {useful["mapid"]}')
            print('Failed going to bank')
            return False
        return True
    ...
    def search(word, type):
        types = {
            ...
            'storage': [491, 0, 0],
            'workshop': [85, 0, -50],
            'hdv inventory': [487, 0, 0],
            'hdv_buy': [99, 0, 0],
            'recipe': [130, 20, -10],
            'hdv_sell': [105, 0, 0],
            'sell_price': [170, 0, -20]
        }
        click(types[type][0], types[type][1], types[type][2], c=3, s=1)
        write_word(word)
        sleep(5)

    def search_inventory(word):
        click(501, c=3, s=1)
        write_word(word)
        sleep(5)

    def search_storage(word):
        click(491, c=3, s=1)
        write_word(word)
        sleep(5)

    def search_workshop(word):
        click(85, 0, -50, c=3, s=1)
        write_word(word)
        sleep(5)

    def use_workshop(skillid):
        workshop_list = [*useful['resources'].values()]
        l = []
        for work in workshop_list:
            if work['enabledSkill'] == skillid:
                l.append(work)
        click(l[randint(0, len(l) - 1)]['elementCellId'], offx=10, offy=-50, s=15, so=30)

    def go_to_workshop(jobid):
        print('going to workshop')
        workshops_nid = {26: {'node': '5282', 'name': 'alchemist'},
                         28: {'node': '1138', 'name': 'farmer'},
                         36: {'node': '6736', 'name': 'fisherman'},
                         2: {'node': '8698', 'name': 'lumberjack'}}
        move(workshops_nid[jobid]['node'], False)

    def craft(item_name, jobid, skillid, qantity=9999):
        print('statring craft')
        go_to_workshop(jobid)
        use_workshop(skillid)
        search_workshop(item_name)
        # display availabe recipes
        click(450, s=2)
        # get first result in search
        click(85, 0, 50, s=2)
        # if useful['craft']['gid'] == 1337:
        #     #check if it is really the desired item
        #     pass
        # change how many
        click(245, s=2)
        if not qantity == 9999:
            write_word(str(qantity))
        press('{ENTER}', s=2)
        # combine
        click(315, 0, -10, s=3)
        # if useful['craft']['result'] == result:
        #     #check the result
        #     pass
        press('{VK_ESCAPE}', s=1)
    ...
    def search_recipe(word):
        print('search in recipe')
        if open_recipe_in_bank():
            search(word, 'recipe')
        else:
            logging.warning('couldnt search in recipes')

    def open_recipe_in_bank():
        print('opening recipe')
        if useful['storage']:
            click(504, 15, -20, s=1)
            return True
        else:
            print('bank dialog not open')
            logging.warning('bank dialog not open')
            return False
    ...
    def transfer_recipe2inventory(quantity):
        print('transfer 2 inv')
        click(177, s=1)
        write_word(str(quantity))
        press('{ENTER}', s=1)

    def open_bank():
        print('opening bank')
        clicking_on_npc()
        # chosing to open bank from dialog
        press('{DOWN}', s=1)
        press('~', s=5)
    ...
    def bank():
        while True:
            print('goin to bank')
            logging.info(f'going to bank')
            move_to_closest_bank()
            open_bank()
            transfer_from_inventory()
            to_craft = check_recipes_in_bank()
            if not to_craft:
                print('no craftable items')
                return False
            search_recipe(to_craft['resultName'])
            transfer_recipe2inventory(to_craft['quantity'])
            close_bank()

            craft(item_name=to_craft['resultName'], jobid=to_craft['jobId'], skillid=to_craft['skillid'],
                  qantity=to_craft['quantity'])
            sleep(5)

    def check_job_level(recipe):
        """
        check if required job level is lower than my job level
        """
        if str(recipe['jobId']) in useful['jobs'].keys():
            if recipe['resultLevel'] <= useful['jobs'][str(recipe['jobId'])]['jobLevel']:
                return True
        return False

    def check_recipe_weight(recipe):
        """
        return quantity of 91% of the recipe weight in inventory
        """
        total_weight = 0
        free_space = useful["inventory_max"] - useful["inventory_weight"]
        free_space = free_space * .91
        for ingredient in recipe['Ingredients']:
            total_weight += id_2_weight[str(ingredient[0])] * ingredient[1]
        quantity = int(free_space / total_weight)
        return quantity

    def check_ingredients_exist(recipe):
        min_recipe_quantity = 99999
        recipe_quantity = 0
        for ingredient in recipe['Ingredients']:
            for x in useful['storage'].values():
                try:
                    if ingredient[0] == x["objectGID"]:
                        if ingredient[1] < x['quantity']:
                            recipe_quantity = int(x['quantity'] / ingredient[1])
                            break
                except:
                    pass
            else:
            ...
            if recipe_quantity < min_recipe_quantity:
                min_recipe_quantity = recipe_quantity
        q = check_recipe_weight(recipe)
        if q < min_recipe_quantity:
            min_recipe_quantity = q
        if min_recipe_quantity == 0:
            return False
        return min_recipe_quantity

    def get_highest_level_recipe(can_craft):
        """
        get the recipe with highest job level
        """
        max_level = 0
        best = None
        for item in can_craft:
            if item['resultLevel'] > max_level:
                max_level = item['resultLevel']
                best = item
        return best

    def get_recipe_skill_id(recipe):
        for skill in skills:
            if skill['parentJobId'] == recipe['jobId']:
                if recipe['resultId'] in skill['craftableItemIds']:
                    return skill['id']

    def check_recipes_in_bank(specified_job='all'):
        can_craft = []
        for recipe in recipes:
            if (specified_job == recipe['jobId']) or (specified_job == 'all'):
                if not check_job_level(recipe):
                    continue
                if q := check_ingredients_exist(recipe):
                    recipe['quantity'] = q
                    recipe['skillid'] = get_recipe_skill_id(recipe)
                    can_craft.append(recipe)
        if can_craft:
            return get_highest_level_recipe(can_craft)
        return False

    def go_to_and_open_hdv(hdv_type, tradetype="buyer"):
        print(' going and openin hdv')
        hdv_dic = {
            'resource': [73400322, 423],  # 73400322
            'Ressources': [146741, 328]
        }

        move(get_current_node(1, hdv_dic[hdv_type][0], pos=17))
        while not useful['dialog']:
            click(hdv_dic[hdv_type][1], s=10)
        if not (useful["buyer_seller"] == tradetype):
            if tradetype == "seller":
                open_seller_in_hdv()

    def open_seller_in_hdv():
        if useful['dialog']:
            click(47, 30, 25, s=5)

    def set_sell_quantity(quantity):
        convert = {1: 0, 10: 1, 100: 2}
        click(170, 0, 20, s=5)
        press('{UP}', s=.5)
        press('{UP}', s=.5)
        press('{UP}', s=.5)
        for _ in range(convert[int(quantity)]):
            press('{DOWN}', s=.5)

    def get_first_item_in_hdv_inventory():
        click(108, s=1)
    ...
    def get_first_item_on_buy_list():
        click(131, -20, -20, s=2, c=1)

    def buy(item, quantity):
        search(item, 'hdv_buy')
        sleep(1)
        # clicking on first item on buy
        get_first_item_on_buy_list()
        q100 = int(quantity / 100)
        q10 = int((quantity % 100) / 10)
        q1 = int(quantity % 10)
        for _ in range(q100):
            buy_item(100)
        for _ in range(q10):
    ...
    # useful['tobuy']['effects']

    def sell(item='Farmland', hdv_type='resource', price=3371, quantity=1):
        go_to_and_open_hdv(hdv_type)
        open_seller_in_hdv()
        search(item, 'hdv inventory')
        get_first_item_in_hdv_inventory()
        # get prices
        set_sell_quantity(quantity)
        search(str(price), 'sell_price')
        put_on_sale()

    def get_price_if_item_already_on_sell_list(gid, quantity):
        for x in useful['seller'].values():
            try:
                if gid == x["objectGID"] and quantity == x["quantity"]:
                    if x["objectPrice"] == useful['tobuy']['prices'][0]:  # TODO make it babsed on quantity
                        return x["objectPrice"]
            except:
                pass
        return useful['tobuy']['prices'][0] - 1

    def craftable(gid):
        for recipe in recipes:
            if gid == recipe['resultId']:
                return recipe
        return False

    def isingrediant(gid):
        for recipe in recipes:
    ...
    def check_if_some_of_those_are_in_bank(recipe, quantity):
        """
        get what ingredians are in bank wich are partialy in bank and wich are not present at all
        inbank, notinbank format = [gid, ingrediant quantity]
        partial format = [gid, ingrediant quantity, missing quantity]
        """
        inbank = []
        partial = []
        notinbank = []
        for ingredient in recipe['Ingredients']:
            for x in useful['storage'].values():
                try:
                    if ingredient[0] == x["objectGID"]:
                        if (ingredient[1] * quantity) < x['quantity']:
                            ingredient[1] = ingredient[1] * quantity
                            inbank.append(ingredient)
                            break
                        elif ingredient[1] < x['quantity']:
                            missing = (ingredient[1] * quantity) - x['quantity']
                            ingredient[1] = missing
                            # ingredient.append(x['quantity'])
                ...
                ingredient[1] = ingredient[1] * quantity
                notinbank.append(ingredient)

        return inbank, partial, notinbank

    def flip(item='Farmland Substrate', hdv_type='resources'):
        go_to_and_open_hdv(hdv_type)
        search(item, 'hdv_buy')
        sleep(1)
        get_first_item_on_buy_list()
        if id_2_names[str(useful['tobuy']['objectGID'])] == item:
            sellprice = useful['tobuy']['prices'][0]
            buy_item(10)
        open_seller_in_hdv()
        search(item, 'hdv inventory')
        get_first_item_in_hdv_inventory()
        set_sell_quantity(1)
        sellprice = get_price_if_item_already_on_sell_list(useful['tobuy']['objectGID'], quantity=1)
        search(str(sellprice), 'sell_price')
        for _ in range(5):
            put_on_sale()
        print(useful)
        logging.error(f'useful {useful}')

    def check_if_at_correct_hdv(gid):
        """
        check if at correct hdv else go to that hdv
        """
        print('check if at correct hdv else go to that hdv')
        if useful["buyer_seller"] == True:
            if id_2_type[str(gid)] in useful['seller']['types']:
                # TODO: add if it's in seller or buyer mode and change
                return True
        if useful['dialog'] == True:
            press('{VK_ESCAPE}', s=1)
        for k, v in hdv_2_id.items():
            if gid in v:  # id_2_type[str(gid)] ????
                go_to_and_open_hdv(k, tradetype="buyer")

    def buyitems(itemslist):
        print(' buyin multiple items ', itemslist)
        for item in itemslist:
            check_if_at_correct_hdv(item[0])
            buy(id_2_names[str(item[0])], item[1])
        if useful["dialog"] == True:
            press('{VK_ESCAPE}', s=1)

    def craft_after_checking_bank(oneitem, quantity, buy=True):
        print('crafting after checkin bank')
        # always get what inventory can hold 90%
        if quantity < 1:
            return True
        if type(oneitem) is not dict:
            tocraft = check_specific_recipe_in_bank(oneitem,
                                                    quantity)  # [item, quanitity] quanitty is based on inventory, None if not
        else:
            tocraft = [oneitem, quantity]  # now its always this one
        print(' to cratf ', tocraft)
        recipe_quantity = check_recipe_weight(tocraft[0])
        yes, partial, no = check_if_some_of_those_are_in_bank(tocraft[0], recipe_quantity)
        tocheck = partial + no
        for item in tocheck:
            if recipe := craftable(item):
                craft_after_checking_bank(tocraft[0], tocraft[1], buy)
            else:
                if buy:
                    buyitems([
                        item])  # buy a list if int turn to name, if in right hdv stay, else close if in dialog and go to hdv
                else:
                    print('cant buy so exit')
                    logging.error('cant buy so exit')
                    return False
        quantity -= tocraft[1]
        ...
        open_bank()
        transfer_from_inventory()
        search_recipe(tocraft[0]['resultName'])
        transfer_recipe2inventory(tocraft[1])
        # TODO: sometimes the item is not first so i need to check, or filter by profession, maybe better
        close_bank()

        craft(item_name=tocraft[0]['resultName'],
              jobid=tocraft[0]['jobId'],
              skillid=tocraft[0]['skillid'],
              qantity=tocraft[1])
        print('crafted with success')
        if not craft_after_checking_bank(oneitem, quantity):
            print('checking new recipe ', oneitem, quantity)
            return False
    ...
    def jobxp(option=1):
        ...
        while True:
            ...
            if option == 1:  # normal
                recipe = check_recipes_in_bank(specified_job='all')
                buy = False
            elif option == 2:  # im rich
                recipe = check_max_level_recipe_i_can_craft()
                buy = True
            if recipe == False:
                continue
            print('slected recipe is ', recipe)
            quantity = check_recipe_weight(recipe)
            craft_after_checking_bank(recipe, quantity, buy)  # recipe['quantity']


    def check_max_level_recipe_i_can_craft(specified_job='all'):
        print('checkin max level i can craft')
        can_craft = []
        for recipe in recipes:
            if (specified_job == recipe['jobId']) or (specified_job == 'all'):
                if not check_job_level(recipe):
    ...
    def check_specific_recipe_in_bank(oneitem, quantity):
        for recipe in recipes:
            if oneitem == recipe['resultName']:
                if not check_job_level(recipe):
                    print(' cant craft this ')
                    logging.error('cant craft this')
                if q := check_ingredients_exist(recipe):
                    if q == 0:
                        return False
                    recipe['quantity'] = q
    ...
    if useful['mount'] and 'riding' in useful['mount'] and useful['mount']['riding']:    press('{VK_NUMPAD6}', s=1,
                                                                                               so=1)
    for x in ' /solo~':    press(x, s=.2)
    if (cond := parameters[0] == '1' and parameters[7] != '3') or parameters[0] == '2':
        g, param = lambda rsc: [s for r in rsc for x in
                                collection.skills.find({'_id': {'$regex': '^%s.*' % (r), '$options': 'i'}}) for s in
                                x['skill_id']], [[y.replace(' ', '') for y in x[x.find(':') + 1:].split(',')] for x in
                                                 parameters[7:]]
        param_list = [[x for x in collection.paths.find(
            {'$or': [{'_id': {'$regex': f'.*{x}.*', '$options': 'i'}} for x in param[0]]}, {'zaap': 1, 'nodes': 1})],
                      set(g(param[1])), g(param[2]), {x['_id']: [x['offx'], x['offy']] for x in
                                                      collection.exceptions.find({}, {'offx': 1, 'offy': 1})},
                      parameters[7] == '1']
    treasure_hunt(0, param_list if cond else False) if parameters[0] == '1' else harvest(*param_list) if parameters[
                                                                                                             0] == '2' else level_up(
        parameters[-1]) if parameters[0] == '3' else explore() if parameters[0] == '4' else seller() if parameters[
                                                                                                            0] == '5' else logging.error(
        'Wrong algorithm option')
    if thread:
        connected = False
        thread.join()
