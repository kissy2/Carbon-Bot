...
prev, get_hwnd, get_window, bti, conn, nl, server = None, lambda hwnd: Application().connect(
    handle=hwnd.handle).top_window(), lambda text: find_elements(title_re=f'^{text}'), \
                                                    lambda b: int.from_bytes(b, 'big', signed=True), connect(
    'cbd', isolation_level=None), '\n\n', (gethostbyname(gethostname()), 16969)
conn.cursor().execute(
    'create table if not exists accounts(login primary key,password not null,name not null,server not null)')
...
def hook(login, password, name):
    try:
        ...
        while Popen(args='REG QUERY HKCU\Environment /v eca', stdout=PIPE, stderr=DEVNULL).communicate()[0][:-1][
            -2] == '1':    sleep(2)
...
def sniffer(p, conn):
    try:
        ...
        if next_seq and seq != next_seq:
            ...
            for i in wait:
                conn.sendto(len(i[1]).to_bytes(2, 'big') + i[1], server)
...
def post_hook(window, s, addr, check=True):
    try:
        ...
        for x in Popen("netstat -no", stdout=PIPE, stderr=DEVNULL).communicate()[0][:-1].split(b'\r\n')[4:]:
            ...
            return None, app, None
        sniff = AsyncSniffer(filter=f'tcp and dst port {client_port} and host {addr.decode()}',
                             lfilter=lambda p: p.haslayer(Raw), prn=lambda p: sniffer(p, s))
        sniff.start()
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        try:
            if (i := input(
                    f'{nl[0]}Welcome to carbon bot choose one of the options below :{nl}1 - Launch all accounts in the database{nl}2 - Launch specific accounts from the database{nl}3 - Add new accounts to the database{nl}4 - Delete accounts from the database{nl}5 - Show existing accounts{nl}>> ')) == '1':
                for row in db.execute('select * from accounts'):
                    execution_pool[f'{row[2]}-{row[3]}'] = new_session(row)
                break
            elif i == '2':
                prn_accounts()
                for row in db.execute("select * from accounts where rowid in {}".format(tuple(t) if len(t := input(
                        f"Enter the Ids (seperated by a comma ',' ) of the accounts you want to run{nl}>> ").split(
                    ',')) > 1 else f'({t[0]})')):
                    execution_pool[f'{row[2]}-{row[3]}'] = new_session(row)
                break
            elif i == '3':
                ...
                while 1:
                    if first or (i := input(f'{nl}Add another account ? : y/n{nl}>> ').lower()) == 'y':
                        db.execute('insert into accounts values(?,?,?,?)', (
                            input(f"{nl[0]}Enter account name : "), input(f"{nl[0]}Enter account password : "),
                            input(f"{nl[0]}Enter character name : "), input(f"{nl[0]}Enter server name : ")))
                        first = False
                    elif i == 'n':
                        break
            elif i == '4':
                prn_accounts()
                db.execute('delete from accounts where rowid=' + input(
                    f'Enter the id of the account you want to delete from the database{nl}>> '))
